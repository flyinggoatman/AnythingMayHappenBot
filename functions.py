...
async def check_mysql_connection(SQL_HOST, SQL_USER, SQL_PASS, SQL_DATABASE, bot):
    warning_channels = [950141127565144104, 1211367513674227732]
    try:
        engine = create_engine(f"mysql+pymysql://{SQL_USER}:{SQL_PASS}@{SQL_HOST}/{SQL_DATABASE}")
        connection = engine.connect()
        connection.close()
        print("MySQL connection is working.")
        for channel_id in warning_channels:
            channel = bot.get_channel(channel_id)
            if channel:
                await channel.send("Bot has fully booted up and the MySQL connection is working.", delete_after=5)
    except Exception as e:
        print(f"Failed to connect to MySQL: {e}")
        for channel_id in warning_channels:
            channel = bot.get_channel(channel_id)
            if channel:
                await channel.send(f"Failed to connect to MySQL: {e}", delete_after=20)
...
async def add_new_link(link, session, Link, message, bot, DEBUG_MODE, command_tag, url, author, DISCORD_CHANNEL_ID_int, GITHUB_CHANNEL_int, BLENDER_ADDONS_CHANNEL_int, ALL_CATEGORIES_CHANNEL_ID_int, classified_user, PHONE_APPS_CHANNEL_int, VIDEOS_CHANNEL_int, EXTENSION_CHANNEL_int, SUBREDDIT_CHANNEL_int, EBOOKS_PDF_CHANNEL_int, VIDEO_GAMES_CHANNEL_int, AI_WEBSITE_CHANNEL_int, SOFTWARE_CHANNEL_int, mention, discord_name):
    ...
    # video games
        ...
        await temp_channel.send(f"We have added the video game successfully. Please check <#{VIDEO_GAMES_CHANNEL_int}> for your submitted link.", delete_after=5)
        if author.id in classified_user:
        ...
        temp_channel = bot.get_channel(DISCORD_CHANNEL_ID_int)
        await temp_channel.send(f"We have added the blender addon successfully. Please check <#{BLENDER_ADDONS_CHANNEL_int}> for your submitted link.", delete_after=5)
        if author.id in classified_user:
        ...
        temp_channel = bot.get_channel(DISCORD_CHANNEL_ID_int)
        await message.channel.send(f"We have added the AI Website successfully. Please check <#{AI_WEBSITE_CHANNEL_int}> for your submitted link.", delete_after=5)
        if author.id in classified_user:
        ...
        temp_channel = bot.get_channel(DISCORD_CHANNEL_ID_int)
        await temp_channel.send(f"We have added the GitHub Repo successfully. Please check <#{GITHUB_CHANNEL_int}> for your submitted link.", delete_after=5)
        if author.id in classified_user:
        ...
        temp_channel = bot.get_channel(DISCORD_CHANNEL_ID_int)
        await temp_channel.send(f"We have added the E-Book successfully. Please check <#{EBOOKS_PDF_CHANNEL_int}> for your submitted link.", delete_after=5)
        if author.id in classified_user:
        ...
        if message.channel.id != PHONE_APPS_CHANNEL_int:
            await temp_channel.send(f"We have added the phone app successfully. Please check <#{PHONE_APPS_CHANNEL_int}> for your submitted link.", delete_after=5)
        if author.id in classified_user:
        ...
        temp_channel = bot.get_channel(DISCORD_CHANNEL_ID_int)
        await temp_channel.send(f"We have added the extension successfully. Please check <#{EXTENSION_CHANNEL_int}> for your submitted link.", delete_after=5)
        if author.id in classified_user:
            await target_channel.send(f"Extension {url} added successfully by **user classified**! Please keep in mind extensions may not be verified by the Chrome Web Store, Mozilla Addons, Opera Addons, or Microsoft Edge Addons.")
            await all_target_channel.send(f"Extension {url} added successfully by **user classified**! Please keep in mind extensions may not be verified by the Chrome Web Store, Mozilla Addons, Opera Addons, or Microsoft Edge Addons.")
        else:
            await target_channel.send(f"Extension {url} added successfully by **{author.display_name}**! Please keep in mind extensions may not be verified by the Chrome Web Store, Mozilla Addons, Opera Addons, or Microsoft Edge Addons.")
            await all_target_channel.send(f"Extension {url} added successfully by **{author.display_name}**! Please keep in mind extensions may not be verified by the Chrome Web Store, Mozilla Addons, Opera Addons, or Microsoft Edge Addons.")

    elif re.search(r"reddit\.com/r/", url) and not re.search(r"/comments/", url):
        target_channel = bot.get_channel(SUBREDDIT_CHANNEL_int)
        temp_channel = bot.get_channel(DISCORD_CHANNEL_ID_int)
        await temp_channel.send(f"We have added the subreddit successfully. Please check <#{SUBREDDIT_CHANNEL_int}> for your submitted link.", delete_after=5)
        if author.id in classified_user:
        ...
        temp_channel = bot.get_channel(DISCORD_CHANNEL_ID_int)
        await temp_channel.send(f"We have added the YouTube video successfully. Please check <#{VIDEOS_CHANNEL_int}> for your submitted link.", delete_after=5)
        if author.id in classified_user:
        ...
        temp_channel = bot.get_channel(DISCORD_CHANNEL_ID_int)
        await temp_channel.send(f"We have added the software successfully. Please check <#{SOFTWARE_CHANNEL_int}> for your submitted link.", delete_after=5)
        if author.id in classified_user:
